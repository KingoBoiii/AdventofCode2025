...
{
    internal static string SumGrandTotal(string worksheet)
    {
        ...
        while (cIndex < maxLen)
        {
            ...
            {
                var rowSlice = lines[r].Substring(start, end - start + 1).Trim();
                if (rowSlice.Length == 0)
                {
                    continue;
                }

                if (long.TryParse(rowSlice, out var n))
                {
                    numbers.Add(n);
                }
                else
                {
                    // If parsing fails, ignore that row (keeps behavior forgiving)
                    continue;
                }
            }
